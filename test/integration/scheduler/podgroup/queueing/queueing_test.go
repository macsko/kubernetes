...
func TestPodGroupQueueing(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "gang is scheduled, then pod group is deleted, and new enqueued pods belong to it end up in incomplete entities",
			steps: []stepsframework.Step{
				{
				...
				{
					Name:                            "Verify the new pod ends up in incomplete entities",
					WaitForPodsInIncompleteEntities: []string{"p3"},
				},
			},
		},
		{
			name: "basic group is scheduled, then pod group is deleted, and new enqueued pods belong to it end up in incomplete entities",
			steps: []stepsframework.Step{
				{
		...
		{
			name: "gang is partially scheduled, then pod group is deleted, unschedulable pods end up in incomplete entities",
			steps: []stepsframework.Step{
				{
					Name:           "Create the PodGroup object",
					CreatePodGroup: podGroupGang,
				...
				{
					Name:                            "Verify p3 ends up in incomplete entities",
					WaitForPodsInIncompleteEntities: []string{"p3"},
				},
			},
		},
		{
			name: "basic group is partially scheduled, then pod group is deleted, unschedulable pods end up in incomplete entities",
			steps: []stepsframework.Step{
				{
					Name:           "Create the PodGroup object with Basic policy",
					CreatePodGroup: podGroupBasic,
		...
		{
			name: "gang is unschedulable, then pod group is deleted, unschedulable pods end up in incomplete entities",
			steps: []stepsframework.Step{
				{
					Name:           "Create the PodGroup object",
				...
				{
					Name:                            "Verify gang pods end up in incomplete entities",
