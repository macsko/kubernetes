		...
		for _, asyncAPICallsEnabled := range []bool{true, false} {
			for _, item := range table {
				t.Run(fmt.Sprintf("%s (Queueing hints enabled: %v, Async API calls enabled: %v)", item.name, qHintEnabled, asyncAPICallsEnabled), func(t *testing.T) {
					...
					var apiCache *apicache.APICache
					...
					internalCache := internalcache.New(ctx, 30*time.Second)
			...
			t.Run(fmt.Sprintf("%s (Async API calls enabled: %v)", test.name, asyncAPICallsEnabled), func(t *testing.T) {
				...
				fwk, err := tf.NewFramework(ctx,
					[]tf.RegisterPluginFunc{
						tf.RegisterQueueSortPlugin(queuesort.Name, queuesort.New),
						tf.RegisterBindPlugin(defaultbinder.Name, defaultbinder.New),
					}, "", frameworkruntime.WithClientSet(client), frameworkruntime.WithAPIDispatcher(apiDispatcher), frameworkruntime.WithAPICacher(apiCache), frameworkruntime.WithEventRecorder(&events.FakeRecorder{}))
				if err != nil {
					t.Fatal(err)
				}
		...
		for _, test := range tests {
			t.Run(fmt.Sprintf("%s (Async API calls enabled: %v)", test.name, asyncAPICallsEnabled), func(t *testing.T) {
				...
				if asyncAPICallsEnabled {
					...
					apiDispatcher.Run(logger)
					defer apiDispatcher.Close()
					apiCacher = apicache.New(apiDispatcher)
				}
