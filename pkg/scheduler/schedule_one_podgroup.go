...
func (sched *Scheduler) frameworkForPodGroup(podGroupInfo *framework.QueuedPodGroupInfo) (framework.Framework, error) {
	...
	for _, pInfo := range podGroupInfo.QueuedPodInfos {
		if schedulerName == "" {
			...
			return nil, fmt.Errorf("all pods in a single pod group should have the same .spec.schedulerName set, got: %q and %q", pInfo.Pod.Spec.SchedulerName, schedulerName) // TODO: Move to coscheduling plugin?
		}
	}
