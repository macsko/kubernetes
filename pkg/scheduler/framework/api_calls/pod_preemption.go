...
// PodPreemptionCall is used to preempt the victim by updating a condition and deleting it.
type PodPreemptionCall struct {
	// victimUID is an UID of a victim pod.
	victimUID types.UID
	// victimRef is a reference to a victim pod.
	victimRef klog.ObjectRef
	// victimStatus contains the actual status of the victim pod.
	victimStatus *v1.PodStatus
	// preemptorRef is a reference to a preemptor pod and is used only for the logging purposes.
	preemptorRef klog.ObjectRef
	// newCondition is a condition to set before deleting a victim.
	newCondition *v1.PodCondition
}

func NewPodPreemptionCall(victim *v1.Pod, preemptor *v1.Pod, newCondition *v1.PodCondition) *PodPreemptionCall {
	return &PodPreemptionCall{
		victimUID:    victim.UID,
		victimRef:    klog.KObj(victim),
		preemptorRef: klog.KObj(victim),
		victimStatus: victim.Status.DeepCopy(),
...
func (ppc *PodPreemptionCall) Execute(ctx context.Context, client clientset.Interface) error {
	...
	newStatus := ppc.victimStatus.DeepCopy()
	updated := podutil.UpdatePodCondition(newStatus, ppc.newCondition)
	if updated {
		if _, err := util.PatchPodStatus(ctx, client, ppc.victimRef.Name, ppc.victimRef.Namespace, ppc.victimStatus, newStatus); err != nil {
