...
func (sched *Scheduler) updatePodInSchedulingQueue(oldObj, newObj interface{}) {
	...
	if isAssumed {
		assumedPod, err := sched.Cache.GetPod(newPod)
		if err == nil && newPod.DeletionTimestamp != nil && oldPod.DeletionTimestamp == nil { // TODO: Race when the pod was bound but we didn't update the cache? Updates are serialized, but verify. We also can just lose one, right?
			...
			sched.handleAssumedPodDeletion(assumedPod)
		}
		return
	}
