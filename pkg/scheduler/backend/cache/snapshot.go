...
// NewSnapshotWithPodGroups initializes a Snapshot struct with pod groups and returns it.
func NewSnapshotWithPodGroups(pods []*v1.Pod, nodes []*v1.Node, podGroups []*schedulingv1alpha3.PodGroup) *Snapshot {
	s := NewSnapshot(pods, nodes)
	for _, podGroup := range podGroups {
		s.podGroups[newPodGroupKey(podGroup.Namespace, podGroup.Name)] = podGroup
	}
	return s
}
