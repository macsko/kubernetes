...
type invalidStatusUpdate struct {
...
func (p *PriorityQueue) validateAndFlushInvalidHierarchies(logger klog.Logger) {
	...
	var statusUpdates []invalidStatusUpdate
	for _, record := range newlyInvalid {
		...
		for memberKey := range record.memberKeys {
			...
			if p.client != nil {
				...
				if gpg, ok := p.workloadForest.getByEntityKey(memberKey); ok {
					if gpg.PodGroup != nil {
						update.pg = gpg.PodGroup.DeepCopy()
					} else if gpg.CompositePodGroup != nil {
						update.cpg = gpg.CompositePodGroup.DeepCopy()
					}
				}
	...
	for _, target := range statusUpdates {
		p.patchInvalidStatus(ctx, logger, target)
	}
}

func (p *PriorityQueue) patchInvalidStatus(ctx context.Context, logger klog.Logger, u invalidStatusUpdate) {
	if p.client == nil {
		return
	}
	...
	if u.pg != nil {
		...
		condition.ObservedGeneration = u.pg.Generation
		newStatus := u.pg.Status.DeepCopy()
		if apimeta.SetStatusCondition(&newStatus.Conditions, condition) {
			if err := util.PatchPodGroupStatus(ctx, p.client, u.pg.Name, u.pg.Namespace, &u.pg.Status, newStatus); err != nil {
				utilruntime.HandleErrorWithLogger(logger, err, "Failed to patch invalid status on PodGroup", "podGroup", klog.KObj(u.pg))
			}
		}
	} else if u.cpg != nil {
		condition.Type = schedulingv1alpha3.CompositePodGroupInitiallyScheduled
		condition.ObservedGeneration = u.cpg.Generation
		newStatus := u.cpg.Status.DeepCopy()
		if apimeta.SetStatusCondition(&newStatus.Conditions, condition) {
			if err := util.PatchCompositePodGroupStatus(ctx, p.client, u.cpg.Name, u.cpg.Namespace, &u.cpg.Status, newStatus); err != nil {
				utilruntime.HandleErrorWithLogger(logger, err, "Failed to patch invalid status on CompositePodGroup", "compositePodGroup", klog.KObj(u.cpg))
			}
		}
	}
}
