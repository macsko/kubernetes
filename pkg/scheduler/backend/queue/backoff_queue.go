...
func (bq *backoffQueue) calculateBackoffDuration(count int, entitySize int) time.Duration {
	...
	// Multiply the maximum backoff duration by the number of pods in the entity.
	maxBackoff := bq.podMaxBackoff * time.Duration(entitySize)
...
func (bq *backoffQueue) add(logger klog.Logger, entity framework.QueuedEntityInfo, event string) {
	...
	if entity.GetQueueingParams().UnschedulablePlugins.Len() == 0 && entity.GetQueueingParams().PendingPlugins.Len() == 0 {
		...
		err := bq.entityBackoffQ.Delete(entity)
		if err == nil {
	...
	err := bq.entityErrorBackoffQ.Delete(entity)
	if err == nil {
...
func (bq *backoffQueue) delete(entity framework.QueuedEntityInfo) bool {
	...
	if bq.entityBackoffQ.Delete(entity) == nil {
		return true
	}
	return bq.entityErrorBackoffQ.Delete(entity) == nil
}
...
func (bq *backoffQueue) has(entity framework.QueuedEntityInfo) bool {
	...
	return bq.entityBackoffQ.Has(entity) || bq.entityErrorBackoffQ.Has(entity)
}
