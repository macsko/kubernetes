...
type incompleteEntities struct { // TODO: Unit tests
...
// has checks if there are any pod infos waiting for the pod group.
func (p *incompleteEntities) has(pgKey string) bool {
	_, ok := p.podGroupToPodInfos[pgKey]
	return ok
}
...
// delete removes and returns all pod infos waiting for the pod group.
func (p *incompleteEntities) delete(pgKey string) []*framework.QueuedPodInfo {
...
// update updates the pod inside the incomplete entities and returns true if found.
func (p *incompleteEntities) update(pod *v1.Pod) bool {
	key := podGroupKeyForPod(pod)
...
// deletePod removes a specific pod and returns its QueuedPodInfo.
// If the pod list for the group becomes empty, it cleans up the key.
func (p *incompleteEntities) deletePod(pod *v1.Pod) *framework.QueuedPodInfo {
	key := podGroupKeyForPod(pod)
	pInfos, ok := p.podGroupToPodInfos[key]
	if !ok {
		return nil
